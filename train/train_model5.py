...
from net.backbones import load_vgg16_backbone
from net.models import build_model5
...
def main():
    ...
    backbone = load_vgg16_backbone(input_shape=(opt.img_size, opt.img_size, 3))
    model = build_model5(backbone, num_classes=len(opt.classes))
    model.build(input_shape=(None, opt.img_size, opt.img_size, 3))
    model.summary()

    train_and_save(
        model, "model5_vgg16", train_generator, validation_generator,

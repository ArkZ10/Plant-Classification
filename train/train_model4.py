...
from net.models import build_model4
...
def main():
    ...
    model = build_model4(backbone, num_classes=len(opt.classes))
    model.build(input_shape=(None, opt.img_size, opt.img_size, 3))
    model.summary()

    train_and_save(
        model, "model4_inception_leaky", train_generator, validation_generator,
